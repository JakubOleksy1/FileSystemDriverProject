...
NTSTATUS CreateRoutine(PDEVICE_OBJECT DeviceObject, PIRP Irp) {
    ...
    // Handle file creation/opening logic here
...
NTSTATUS CloseRoutine(PDEVICE_OBJECT DeviceObject, PIRP Irp) {
    ...
    // Handle file closing logic here
    // For simplicity, assume success
    Irp->IoStatus.Status = STATUS_SUCCESS;
    IoCompleteRequest(Irp, IO_NO_INCREMENT);
    return STATUS_SUCCESS;
...
NTSTATUS ReadRoutine(PDEVICE_OBJECT DeviceObject, PIRP Irp) {
    ...
    // For simplicity, assume success and return dummy data
...
NTSTATUS QueryInformationRoutine(PDEVICE_OBJECT DeviceObject, PIRP Irp) {
    ...
    // Handle querying file attributes logic here
    // For simplicity, assume success and return dummy attributes
...
NTSTATUS DirectoryControlRoutine(PDEVICE_OBJECT DeviceObject, PIRP Irp) {
    ...
    // Handle listing files logic here
    // For simplicity, assume success and return dummy directory entry
    PFILE_DIRECTORY_INFORMATION dirInfo = (PFILE_DIRECTORY_INFORMATION)Irp->AssociatedIrp.SystemBuffer;

    RtlZeroMemory(dirInfo, sizeof(FILE_DIRECTORY_INFORMATION));
    dirInfo->FileNameLength = sizeof(L"dummy.txt") - sizeof(WCHAR);
    RtlCopyMemory(dirInfo->FileName, L"dummy.txt", dirInfo->FileNameLength);

    Irp->IoStatus.Status = STATUS_SUCCESS;
    Irp->IoStatus.Information = sizeof(FILE_DIRECTORY_INFORMATION) + dirInfo->FileNameLength;
